...
def get_linkedin_job_ids(keyword, time_filter, save_path):
    ...
    while True:
        ...
        for job in soup.find_all("li"):
            ...
            if job_id and time_posted:
                ...
                total_minutes = int(time_text.split()[0]) * 60 if "hour" in time_text else int(time_text.split()[0])

                job_data.append({"Job ID": job_id, "Time Posted": time_text, "Minutes": total_minutes})

        if not job_data:
            print(f"No jobs found on page {start_count}. Stopping pagination.")
    ...
    return filter_job_ids(all_jobs, time_filter, save_path)


def filter_job_ids(job_data, time_filter, save_path):
    """
    ...
    return scrape_job_details(filtered_ids, save_path)


def scrape_job_details(job_ids, save_path):
    """
    ...
    print(f"Job data saved in {save_path}")
    return job_list


# Example Usage: Fetch jobs posted in the last 3 hours (180 minutes) and save to JSON
get_linkedin_job_ids("Data Engineer", time_filter=65, save_path="linkedin_jobs.json")
